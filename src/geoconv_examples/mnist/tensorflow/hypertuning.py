...
from geoconv_examples.mnist.tensorflow.training import MNISTClassifier
...
def hyper_tuning(dataset_path,
    ...
    def build_hypermodel(hp):
        ...
        imcnn = MNISTClassifier(
            ...
            isc_layer_dims=[
                hp.Int(name="isc_layer_1", min_value=8, max_value=256, step=8),
                hp.Int(name="isc_layer_2", min_value=8, max_value=256, step=8),
            ]
        )
        imcnn.compile(
    ...
    tuner.search(x=train_data, validation_data=test_data, epochs=100, callbacks=[stop])
