...
def align_to_partnet(
    ...
    Args:
        ...
        aligned_ShapeNet_target_path (str, optional): Root path to where the mesh should be saved to. Defaults to ./PartNetAligned.
        verbose (bool, optional): Whether to print model statistics after transformations were applied. Defaults to False.
        show_meshes (bool, optional): Whether overlayed meshes should be displayed for inspection purposes. Defaults to False.
    ...
    def shapenet_trans(verts : np.ndarray) -> np.ndarray:
        """Computes the transform from the given mesh to a unit xy-diagonal and axis centered mesh and returns the inverted transform.
        ...
        """
        x_min = np.min(verts[:, 0]); x_max = np.max(verts[:, 0]); x_center = (x_min + x_max) / 2; x_len = x_max - x_min;
        y_min = np.min(verts[:, 1]); y_max = np.max(verts[:, 1]); y_center = (y_min + y_max) / 2; y_len = y_max - y_min;
        z_min = np.min(verts[:, 2]); z_max = np.max(verts[:, 2]); z_center = (z_min + z_max) / 2; z_len = z_max - z_min;
    ...
    def transform_verts(trans : np.ndarray, in_verts : np.ndarray, in_faces : np.ndarray, rot : np.ndarray = None) -> trimesh.Trimesh:
    ...
    if isinstance(tmp_mesh, trimesh.Scene):
        shapenet_mesh = trimesh.util.concatenate(tuple(trimesh.Trimesh(vertices=g.vertices, faces=g.faces) for g in tmp_mesh.geometry.values()))
    ...
    if chamfer_dist > 0.1:
        ...
        shapenet_mesh_t = transform_verts(trans, shapenet_mesh.vertices, shapenet_mesh.faces, rot_90)
        shapenet_pts  = trimesh.sample.sample_surface(shapenet_mesh_t, 2000)[0]
        dist_mat = cdist(shapenet_pts, partnet_pts)
        chamfer_dist = dist_mat.min(0).mean() + dist_mat.min(1).mean()
    ...
    p_x_min = np.min(verts[:, 0]); p_x_max = np.max(verts[:, 0]); p_x_len = p_x_max - p_x_min;
    p_y_min = np.min(verts[:, 1]); p_y_max = np.max(verts[:, 1]); p_y_len = p_y_max - p_y_min;
    p_z_min = np.min(verts[:, 2]); p_z_max = np.max(verts[:, 2]); p_z_len = p_z_max - p_z_min;

    verts = shapenet_mesh_t.vertices
    s_x_min = np.min(verts[:, 0]); s_x_max = np.max(verts[:, 0]); s_x_len = s_x_max - s_x_min;
    s_y_min = np.min(verts[:, 1]); s_y_max = np.max(verts[:, 1]); s_y_len = s_y_max - s_y_min;
    s_z_min = np.min(verts[:, 2]); s_z_max = np.max(verts[:, 2]); s_z_len = s_z_max - s_z_min;
    ...
    if show_meshes:
        ...
        colors = np.concatenate([np.repeat([[0,255,0,255]], len(partnet_mesh.vertices)).reshape(4, -1).T, np.repeat([[255,127,0,255]], len(shapenet_mesh_t.vertices)).reshape(4, -1).T])
...
class PartNetAnnotatedMesh:
    def __init__(self, anno_id : str, base_PartNet_path : str, base_ShapeNet_path : str):
        ...
        self.ShapeNet_model_path = f"{self.base_ShapeNet_path}/{cat_id}/{self.meta['model_id']}/models/model_normalized.obj"
...
def convert_PartNet_labels(
    ...
    c) assigning labels to the transformed ShapeNet mesh by choosing the min distance of signed distance with each individual segment.
...
def manual_filter_mesh(
    ...
    colors = np.concatenate([np.repeat([[0,255,0,255]], len(partnet_mesh.vertices)).reshape(4, -1).T, np.repeat([[255,127,0,255]], len(shapenet_mesh.vertices)).reshape(4, -1).T])
    print(colors.shape, len(mesh.vertices))
    pc = trimesh.PointCloud(vertices=mesh.vertices, colors=colors)
    pc.show()
...
def orig_segmentation(
    ...
    verts = o3d.core.Tensor(shapenet_mesh.vertices, dtype=o3d.core.Dtype.Float32) # sadly no cuda support for embree yet , device=o3d.core.Device("CUDA:0"))
    distances = []
    lbls = []
