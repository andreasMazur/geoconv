from geoconv.pytorch.utils.tensor_utils import (
...
@torch.jit.script
def determine_central_values(
    start: float, stop: float, n_bins: int
...
def shot_descr(
    ...
    -------
    torch.Tensor:
        A rank-3 tensor of shape (n_vertices, azimuth_bins * elevation_bins * radial_bins * histogram_bins) containing
        the SHOT-descriptor for each vertex.
    """
    ########################################################################
    ...
    radial_histogram = histogram_fixed_width_bins(
        v_radial,
        torch.tensor([0.0, radius], device=v_radial.device),
        n_bins=radial_bins,
    )
    elevation_histogram = histogram_fixed_width_bins(
        v_elevation,
        torch.tensor([0.0, torch.pi], device=v_elevation.device),
        n_bins=elevation_bins,
    )
    azimuth_histogram = histogram_fixed_width_bins(
        v_azimuth,
        torch.tensor([0.0, 2 * torch.pi], device=v_azimuth.device),
        n_bins=azimuth_bins,
    )
    binned_vertices = torch.stack(
        [azimuth_histogram, elevation_histogram, radial_histogram], dim=-1
    )
    ...
    cosine_bins = histogram_fixed_width_bins(
        cosines, torch.tensor([-1.0, 1.0], dtype=cosines.dtype), n_bins=histogram_bins
    )
    ...
    cosine_bins = torch.concatenate(
        [binned_vertices, cosine_bins.unsqueeze(-1)], dim=-1
    )
    cosine_bins = torch.concatenate(
        [
            torch.arange(neighborhood_shape[0], device=cosine_bins.device)
            .reshape(neighborhood_shape[0], 1, 1)
            .repeat(1, neighborhood_shape[1], 1),
            cosine_bins,
        ],
        dim=-1,
    )
    ...
    d = (
        torch.abs(cosines - central_values[cosine_bins[:, :, histogram_colon]])
        / step_size
    )
    ...
    closest_neighbor = torch.topk(
        -torch.square(cosines.unsqueeze(-1) - central_values), k=2
    )[1][:, :, 1]
    ...
    tensor_scatter_nd_add_(
        histogram,
        torch.concatenate(
            [cosine_bins[:, :, :histogram_colon], closest_neighbor.unsqueeze(-1)],
            dim=-1,
        ),
        1 - d,
    )
    ...
    ################################
    central_values, step_size = determine_central_values(
        start=0.0, stop=2 * torch.pi, n_bins=azimuth_bins
    )
    d = (
        torch.abs(v_azimuth - central_values[cosine_bins[:, :, azimuth_colon]])
        / step_size
    )
    ...
    # Determine the closest other bin
    closest_neighbor = torch.topk(
        -torch.square(v_azimuth.unsqueeze(-1) - central_values), k=2
    )[1][:, :, 1]
    ...
    ##################################
    central_values, step_size = determine_central_values(
        start=0.0, stop=torch.pi, n_bins=elevation_bins
    )
    d = (
        torch.abs(v_elevation - central_values[cosine_bins[:, :, elevation_colon]])
        / step_size
    )
    ...
    # Determine the closest other bin
    closest_neighbor = torch.topk(
        -torch.square(v_elevation.unsqueeze(-1) - central_values), k=2
    )[1][:, :, 1]
    ...
    d = (
        torch.abs(v_radial - central_values[cosine_bins[:, :, radial_colon]])
        / step_size
    )
    ...
    # Determine the closest other bin
    closest_neighbor = torch.topk(
        -torch.square(v_radial.unsqueeze(-1) - central_values), k=2
    )[1][:, :, 1]
