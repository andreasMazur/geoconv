...
def gamma_func(x):
    ...
    if x - n == 1 / 2:
        return (
            torch.jit._builtins.math.factorial(2 * n)
            / (torch.jit._builtins.math.factorial(n) * 4**n)
        ) * torch.sqrt(torch.pi)
    else:
...
def student_t_pdf(mean_rho, mean_theta, rho, theta, dof):
    ...
    quotient = (
        gamma_func((dof + 1) / 2) / (torch.sqrt(dof * torch.pi) * gamma_func(dof / 2))
    ) ** 2
            ...
            for mean_theta_idx in range(template_matrix.shape[1]):
                ...
                for rho_idx in range(template_matrix.shape[0]):
                    for theta_idx in range(template_matrix.shape[1]):
                        ...
                        ] = student_t_pdf(mean_rho, mean_theta, rho, theta, self.dof)
