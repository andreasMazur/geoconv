from geoconv.pytorch.layers.convolutions.conv_gem import (
    ConvGEM,
    get_kernel_self,
    get_kernel_neigh
)
...
class ConvGEMP(ConvGEM):
    def __init__(self, *args, **kwargs):
        ...
        **kwargs: dict
        ...
        self.V_neigh = nn.Parameter(torch.empty(self.output_dim_halve, self.input_dim_halve, 4, self.n_radial))
        ...
        del self.K_neigh
        self.sine_and_cosine_locs = torch.tensor(
        ...
        del self.K_self
        self.sine_and_cosine_locs_self = torch.tensor(
        ...
        self.phase_weights = nn.Parameter(torch.empty(self.output_dim_halve, 1))
        nn.init.xavier_uniform_(self.phase_weights)
    ...
    def forward_helper(self, signals, template_vertex_interpolations, return_self_and_neighbor_embeddings=False):
        ...
        tuple:
            ...
            (not aggregated over yet). The tensors have shape:
            [
