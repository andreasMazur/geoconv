...
def shot_descr(
    ...
    radial_histogram = tf.histogram_fixed_width_bins(v_radial, [0.0, radius], nbins=radial_bins)
    elevation_histogram = tf.histogram_fixed_width_bins(v_elevation, [0.0, np.pi], nbins=elevation_bins)
    azimuth_histogram = tf.histogram_fixed_width_bins(v_azimuth, [0.0, 2 * np.pi], nbins=azimuth_bins)
    ...
    cosine_bins = tf.histogram_fixed_width_bins(cosines, [-1.0, 1.0], nbins=histogram_bins)
    ...
    cosine_bins = tf.concat(
        [
            tf.tile(tf.range(neighborhood_shape[0])[:, None, None], multiples=(1, neighborhood_shape[1], 1)),
            cosine_bins
        ],
        axis=-1,
    )
    ...
    d = tf.abs(cosines - tf.gather(central_values, cosine_bins[:, :, histogram_column])) / step_size
    ...
    histogram = tf.tensor_scatter_nd_add(
        ...
        tf.concat(   # (n_vertices, n_neighbors, 5)
            [
    ...
    # 'd': (n_vertices, n_neighbors)
    d = tf.abs(v_azimuth - tf.gather(central_values, cosine_bins[:, :, azimuth_column])) / step_size
    ...
    # 'd': (n_vertices, n_neighbors)
    d = tf.abs( v_elevation - tf.gather(central_values, cosine_bins[:, :, elevation_column])) / step_size
    ...
    # 'd': (n_vertices, n_neighbors)
    d = tf.abs(v_radial - tf.gather(central_values, cosine_bins[:, :, radial_column])) / step_size
