...
def shot_descr(neighborhoods,
    ...
    v_elevation = tf.math.acos(  # machine accuracy sometimes return slightly larger/smaller values than allowed
        tf.clip_by_value(neighborhoods[:, :, 2] / v_radial, clip_value_min=-1., clip_value_max=1.)
    )
    ...
    histogram = tf.zeros(
        shape=(tf.shape(neighborhoods)[0], azimuth_bins, elevation_bins, radial_bins, histogram_bins)
    )
    ...
    ################################
    central_values, step_size = determine_central_values(start=0., stop=2 * np.pi, n_bins=azimuth_bins)
    ...
    ##################################
    central_values, step_size = determine_central_values(start=0., stop=np.pi, n_bins=elevation_bins)
