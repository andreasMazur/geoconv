    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.rescale_input_dim is not None:
            self.downsize_fc = tf.keras.layers.Dense(
                self.rescale_input_dim, activation="relu", name="FC_downsize"
            )
            self.downsize_bn = tf.keras.layers.BatchNormalization(
                axis=-1, name="BN_downsize"
            )
        ...
        for idx in range(len(self.isc_layer_dims)):
            self.do_layers.append(
                tf.keras.layers.Dropout(rate=dropout_rate, name=f"DO_layer_{idx}")
            )
            ...
            self.bn_layers.append(
                tf.keras.layers.BatchNormalization(axis=-1, name=f"BN_layer_{idx}")
            )
