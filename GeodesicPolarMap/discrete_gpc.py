...
def compute_u_ijk_and_angle(vertex_i, vertex_j, vertex_k, u, theta, object_mesh, use_c):
    ...
    **Output**
    ...
    if use_c:
        ...
        A = e_j_norm * e_k_norm * np.sin(compute_vector_angle(e_j, e_k))
        ...
        if radicand <= 0:
            ...
            if x_j < 0 or x_k < 0:
                ...
                phi_kj = compute_vector_angle(vertex_k, vertex_j)
                phi_ij = compute_vector_angle(vertex_i, vertex_j)
                if not phi_kj:
                    j = u_j + blas.dnrm2(e_j)
                    k = u_k + blas.dnrm2(e_k)
                    if j <= k:
...
def compute_distance_and_angle(vertex_i, vertex_j, u, theta, triangle_cache, object_mesh, use_c):
    ...
    **Output**
    ...
    for triangle in considered_triangles:
        ...
        if u[vertex_k] < np.inf:
            u_ijk, phi_i = compute_u_ijk_and_angle(vertex_i, vertex_j, vertex_k, u, theta, object_mesh, use_c)
            updates.append((u_ijk, phi_i))

    if not updates:
        return np.inf, .0, triangle_cache
    else:
        u_ijk, phi_i = min(updates)
        return u_ijk, phi_i, triangle_cache


def compute_idx_angle(source_point, vertex_a, vertex_b, object_mesh, use_c):
    """Calculates the angle between two vertices of a triangle w.r.t. to a source point.

    **Input**

    - The point where the angle is measured.
    - End-point of the first edge.
    - End-point of the second edge.
    - A loaded object mesh.
    - A flag whether to use the c-extension.

    **Output**

    - The angle between the edges `(source_point, vertex_a)` and `(source_point, vertex_b)`

    """
    vector_a = object_mesh.vertices[vertex_a] - object_mesh.vertices[source_point]
    vector_b = object_mesh.vertices[vertex_b] - object_mesh.vertices[source_point]

    if use_c:
        return c_extension.compute_angle(vector_a, vector_b)
    else:
        return compute_vector_angle(vector_a, vector_b)


def compute_vector_angle(vector_a, vector_b):
...
def initialize_neighborhood(source_point, u, theta, object_mesh, graph, use_c):
    ...
    **Output**
    ...
    for idx, neighbor in enumerate(source_point_neighbors):
        theta_neighbors[idx] = compute_idx_angle(source_point, ref_neighbor, neighbor, object_mesh, use_c)
    alpha = theta_neighbors.sum() / (2 * np.pi)
    theta[source_point_neighbors] = alpha * theta_neighbors

    return u, theta, source_point_neighbors, graph
...
def local_gpc(source_point, u_max, object_mesh, use_c, eps=0.000001, triangle_cache=None, graph=None):
    ...
    **Output**
    ...
    theta = np.full((object_mesh.vertices.shape[0],), .0)
    u, theta, source_point_neighbors, graph = initialize_neighborhood(source_point, u, theta, object_mesh, graph, use_c)
    u[source_point] = .0
    ...
    while candidates:
        ...
        for i in j_neighbors:
            ...
            new_u_i, new_phi_i, triangle_cache = compute_distance_and_angle(
                i, j, u, theta, triangle_cache, object_mesh, use_c
            )
