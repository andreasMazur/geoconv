from geoconv.examples.mpi_faust.tensorflow.faust_data_set import load_preprocessed_faust
...
class HyperModel(keras_tuner.HyperModel):
    ...
    def build(self, hp):
        ...
        signal_input = tf.keras.layers.Input(shape=self.signal_dim, name="Signal_input")
        bc_input = tf.keras.layers.Input(shape=(self.kernel_size[0], self.kernel_size[1], 3, 2), name="BC_input")
        ...
        for idx in range(len(self.output_dims)):
            signal = tf.keras.layers.Dropout(rate=0.2)(signal)
            ...
            signal = amp(signal)
            signal = tf.keras.layers.BatchNormalization(axis=-1, name=f"BN_layer_{idx}")(signal)
        ...
        init_lr = 0.0004607
        init_wd = 0.0052094
        model = tf.keras.Model(inputs=[signal_input, bc_input], outputs=[output])
