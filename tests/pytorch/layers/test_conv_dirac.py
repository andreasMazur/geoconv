...
class TestConvDiracSameOutput(unittest.TestCase):
    def setUp(self):
        ...
        weights = self.rs.randn(self.batch_size, self.n_vertices, self.n_radial, self.n_angular, 3)
        ...
        self.torch_layer = ConvDirac(
            ...
            activation='elu',
            rotation_delta=1,
            input_shape=((self.batch_size, self.n_vertices, self.n_features),
                         (self.batch_size, self.n_vertices, self.n_radial, self.n_angular, 3, 2))
        )
        self.tf_layer = TFConvDirac(
            amt_templates=128,
            template_radius=0.7,
            activation='elu',
            rotation_delta=1
        ...
        # Assign kernel
        # self.tf_layer._kernel = tf.cast(self.torch_layer._kernel.detach().numpy(), tf.float32)
        # assert np.allclose(self.tf_layer._kernel.numpy(), self.torch_layer._kernel.detach().numpy()), "Kernel mismatch!"
    ...
    def test_forward_preproc(self):
        ...
        pt_conv_center = torch.einsum('tef,skf->sket', self.torch_layer._template_self_weights, pt_signal)
        tf_conv_center = tf.einsum('tef,skf->sket', self.tf_layer._template_self_weights, self.signal)
        self.assertTrue(
